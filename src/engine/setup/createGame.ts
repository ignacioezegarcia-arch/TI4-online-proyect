...
export function createGame(input: CreateGameInput): GameState {
  ...
  const nonHomeTiles: PlaceableTile[] = input.allTiles
    .filter((t) => !t.homeFaction && !t.isHyperlane && !t.isBlank && t !== mecatolTile)
  ...
  for (const systemId of new Set(Object.values(generated.slotToSystemId))) {
    ...
    systems[systemId] = rawTileToSystemState(raw, systemId);
  }
...
function rawTileToSystemState(t: RawTileEntry, systemId: SystemId): SystemState {
  return {
    systemId,
